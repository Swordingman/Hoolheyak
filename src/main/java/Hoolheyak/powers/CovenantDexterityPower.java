...
public class CovenantDexterityPower extends BasePower {
    public static final String POWER_ID = HoolheyakMod.makeID("CovenantDexterity");

    public CovenantDexterityPower(AbstractCreature owner, int amount) {
    ...
    public static void trigger(AbstractCreature owner) {
        if (owner.hasPower(POWER_ID)) {
            ...
            com.megacrit.cardcrawl.dungeons.AbstractDungeon.actionManager.addToBottom(
                    new ApplyPowerAction(owner, owner, new DexterityPower(owner, amt), amt)
            );
        }
    }
